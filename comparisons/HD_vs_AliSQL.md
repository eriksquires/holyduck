...
**HolyDuck is an independent implementation and, specifically, has no code derived from AliSQL or derivative projects.** They are two independent implementations in a similar problem space with fundamentally different architectures and different design goals.  
...
**HolyDuck** is a MariaDB 11.8.3 storage engine plugin for a DuckDB 1.5.0 back-end. We don't embed or alter either MariaDB or DuckDB. Our work happens in the space in between.  
